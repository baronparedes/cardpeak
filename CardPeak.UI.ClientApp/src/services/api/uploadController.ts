...
export function uploadFile(
    ...
    axios
        .post(API.UPLOAD_FILE, data, config)
        .then(r => {
...
export function processBatch(
    id: number,
    successCallback: (
        data: CardPeak.Entities.ProcessedBatchUpload
    ) => void,
    errorCallback: (e: string) => void
) {
    axios
        .post(API.PROCESS_BATCH(id))
        .then(r => {
            successCallback(
                r.data as CardPeak.Entities.ProcessedBatchUpload
            );
        })
        .catch(reason => {
            try {
...
export function getBatchFileConfig(
    bankId: number,
    successCallback: (
    ...
    axios
        .get(API.GET_BATCH_FILE_CONFIG(bankId))
        .then(r => {
            successCallback(
                r.data as CardPeak.Entities.BatchFileConfiguration
            );
        })
        .catch(reason => {
            if (errorCallback) {
                try {
                    errorCallback(
                        reason.response.data.exceptionMessage
                    );
                } catch (e) {
                    errorCallback(reason.message);
                }
...
export function postBatchFileConfig(
    ...
    axios
        .post(url, { ...batchFileConfiguration })
        .then(r => {
            successCallback(
                r.data as CardPeak.Entities.BatchFileConfiguration
            );
...
export function manageUploads(
    ...
    axios
        ...
        .then(r => {
            if (successCallback) {
                successCallback(
                    r.data as CardPeak.Entities.BatchUpload[]
                );
            }
        })
        .catch(reason => {
