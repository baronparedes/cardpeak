...
export function deactivateAgent(
    ...
    axios
        .post(API.DEACTIVATE_AGENT(agentId))
        .then(r => {
            successCallback();
        })
        .catch(reason => {
...
export function getAgentPayout(
    successCallback: (data: CardPeak.Models.AgentPayoutModel) => void,
    errorCallback: (e: string) => void
) {
    axios
        .get(API.GET_AGENT_PAYOUT)
        .then(r => {
...
export function getAgentDashboard(
    ...
    axios.get(API.GET_AGENT_DASHBOARD(agentId)).then(r => {
        successCallback(r.data as CardPeak.Entities.AgentDashboard);
    });
}

export function getAgentSavings(
    agentId: number,
    successCallback: (data: CardPeak.Entities.AgentSavings) => void
) {
    axios.get(API.GET_AGENT_SAVINGS(agentId)).then(r => {
        successCallback(r.data as CardPeak.Entities.AgentSavings);
    });
}

export function getAgentSavingsByYear(
    agentId: number,
    year: number,
    successCallback: (data: CardPeak.Entities.AgentSavings) => void
) {
    axios.get(API.GET_AGENT_SAVINGS_BY_YEAR(agentId, year)).then(r => {
        successCallback(r.data as CardPeak.Entities.AgentSavings);
    });
}

export function getAgentDashboardFiltered(
    agentId: number,
    startDate?: string,
    endDate?: string,
    successCallback?: (data: CardPeak.Entities.AgentDashboard) => void
    ...
    axios
        .get(API.GET_AGENT_DASHBOARD_FILTER(agentId), {
            params: {
                startDate: startDate,
                endDate: endDate
            }
        ...
        .then(r => {
            if (successCallback) {
                successCallback(
                    r.data as CardPeak.Entities.AgentDashboard
                );
            }
        });
}
...
export function postAgentTransaction(
    ...
    successCallback: (
        data: CardPeak.Entities.DebitCreditTransaction
    ) => void,
    ...
    axios
        ...
        .then(r => {
            successCallback(
                r.data as CardPeak.Entities.DebitCreditTransaction
            );
        })
        .catch(reason => {
            errorCallback(reason.message);
