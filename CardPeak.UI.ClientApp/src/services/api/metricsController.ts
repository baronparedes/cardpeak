...
export const API = {
    ...
    GET_BANK_AMOUNT_DISTRIBUTION_METRICS:
        '/metrics/banks/amountdistribution',
    GET_BANK_AGENT_RANKINGS_METRICS: '/metrics/banks/rankings'
};

export function getMetricsWithParams<T>(
    api: string,
    params: any,
...
export function getMetricsByDateRange<T>(
    api: string,
    startDate: string,
    endDate?: string,
    successCallback?: (data: T) => void,
...
export function getMetricsByYearMonth<T>(
    api: string,
    year: number,
    month?: number,
    successCallback?: (data: T) => void,
