...
export function deleteTransaciton(id: number,
    ...
    axios.post(API.DELETE_TRANSACTION(id))
        .then((r) => {
            deleteComplete(id);
        })
        .catch((reason) => {
...
export function getTransactionsByClient(clientName: string,
    successCallback: (data: CardPeak.Entities.ApprovalTransaction[]) => void,
    errorCallback: (message: string) => void) {

    axios.get(API.GET_TRANSACTIONS_BY_CLIENT(clientName))
        .then((r) => {
...
export function getTransactionsByBatch(batchId: number,
    successCallback: (data: CardPeak.Entities.ApprovalTransaction[]) => void,
    errorCallback: (message: string) => void) {

    axios.get(API.GET_TRANSACTIONS_BY_BATCH(batchId))
        .then((r) => {
            successCallback(r.data as CardPeak.Entities.ApprovalTransaction[]);
        })
        .catch((reason) => {
