...
export function getTeamDashboard(
    ...
    axios
        .get(API.GET_TEAM_DASHBOARD(teamId, year))
        .then(r => {
...
export function deleteAgent(
    ...
    axios
        .post(API.DELETE_AGENT(teamId, agentId))
        .then(r => {
            if (r.status === 200) {
                successCallback();
            } else {
...
export function addAgent(
    ...
    successCallback: (
        data: CardPeak.Entities.TeamDashboardDetail
    ) => void,
    errorCallback?: (error: string) => void
) {
    const config = {
    ...
    axios
        .post(API.ADD_AGENT(teamId), null, config)
        .then(r => {
