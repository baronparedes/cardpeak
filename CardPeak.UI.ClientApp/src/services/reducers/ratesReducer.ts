...
function sortRates(rates: CardPeak.Entities.Rate[]) {
    rates.sort((a, b) => {
        return (
            a.bank.description.localeCompare(b.bank.description) ||
            a.cardCategory.description.localeCompare(
                b.cardCategory.description
            )
        );
    });
}
...
function removeRate(
    state: CardPeak.Models.RatesModel,
    payload: CardPeak.Entities.Rate
) {
...
export default handleActions<CardPeak.Models.RatesModel, any>(
    {
        ...
        [RATE_ACTIONS.SELECT_DEFAULT_RATE]: (state, action) => {
            return {
                ...state,
        ...
        [RATE_ACTIONS.SELECT_DEFAULT_RATE_COMPLETE]: (
            state,
            action
        ) => {
            let payload = action.payload as CardPeak.Entities.Rate[];
        ...
        [RATE_ACTIONS.APPLY_DEFAULT_RATE_NOT_FOUND]: (
            state,
            action
        ) => {
            return {
