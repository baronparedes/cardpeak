...
function sortDetails(data: CardPeak.Entities.TeamDashboardDetail[]) {
    data.sort((a, b) => {
        const one =
            a.teamPlacement.agent.firstName +
            ' ' +
            a.teamPlacement.agent.lastName;
        const two =
            b.teamPlacement.agent.firstName +
            ' ' +
            b.teamPlacement.agent.lastName;
...
function saveTeam(
    state: CardPeak.Models.TeamsModel,
    payload: CardPeak.Entities.Team
) {
...
export default handleActions<CardPeak.Models.TeamsModel, any>(
    {
        ...
        [TEAMS_ACTIONS.SELECT_TEAM_DASHBOARD_START]: (
            state,
            action
        ) => {
            return {
        ...
        [TEAMS_ACTIONS.SELECT_TEAM_DASHBOARD_COMPLETE]: (
            state,
            action
        ) => {
            return {
        ...
        [TEAMS_ACTIONS.REFRESH_TEAM_DASHBOARD_COMPLETE]: (
            state,
            action
        ) => {
            return {
        ...
        [TEAMS_ACTIONS.DELETE_TEAM_COMPLETE]: (state, action) => {
            ...
            let selectedTeamDashboard = {
                ...state.selectedTeamDashboard
            };
            if (
                selectedTeam &&
                selectedTeam.teamId === action.payload
            ) {
        ...
        [TEAMS_ACTIONS.DELETE_AGENT_COMPLETE]: (state, action) => {
            ...
            const detail = state.selectedTeamDashboard.details.find(
                _ => _.teamPlacement.agentId === payload.agentId
            );
            const totalApprovals =
                state.selectedTeamDashboard.totalApprovals -
                detail.totalApprovals;
            const details = state.selectedTeamDashboard.details.filter(
                _ => _.teamPlacement.agentId !== payload.agentId
            );
            let performance = state.selectedTeamDashboard.performance.slice();
            performance.forEach(p => {
                const month = detail.performance.find(
                    _ => _.key === p.key
                );
            ...
            return {
                ...
                selectedTeamDashboard: {
                    ...state.selectedTeamDashboard,
                    memberCount:
                        state.selectedTeamDashboard.memberCount - 1,
                    totalApprovals,
                    performance,
                    details
                }
