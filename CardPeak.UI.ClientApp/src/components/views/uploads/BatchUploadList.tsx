...
type BatchUploadDataList = new () => DataListFiltered<
...
class BatchUploadRowLayout extends React.Component<
    DataItemProps<CardPeak.Entities.BatchUpload> & BatchUploadListProps,
    BatchUploadListState
> {
    constructor(
        ...
        return (
            <Row>
                ...
                <Col sm={1} xsHidden={this.props.isHeader}>
                    {this.props.isHeader
                        ? 'id'
                        : this.props.item.batchId}
                </Col>
                <Col sm={3} xsHidden={this.props.isHeader}>
                    {this.props.isHeader
                        ? 'bank'
                        : this.props.item.bank.description}
                </Col>
                <Col sm={3} xsHidden={this.props.isHeader}>
                    {this.props.isHeader
                        ? 'file'
                        : this.props.item.originalFileName}
                </Col>
                <Col sm={2} xsHidden={this.props.isHeader}>
                    {this.props.isHeader
                        ? 'rows'
                        : this.props.item.processedRecords
                        ? this.props.item.processedRecords
                        : null}
                </Col>
                <Col sm={1} smHidden xsHidden={this.props.isHeader}>
                    ...
                    {!this.props.isHeader &&
                    this.props.item.hasErrors === true ? (
                        <span className="text-highlight text-danger">
                            No
                        </span>
                    ) : null}
                    {!this.props.isHeader &&
                    this.props.item.hasErrors === false ? (
    ...
    return (
        <div>
            <BatchUploadDataList
                ...
                renderHeader={() => {
                    return <BatchUploadRowLayout isHeader />;
                }}
                renderItem={(item, key) => {
                    return (
                        <BatchUploadRowLayout
                            {...(props as BatchUploadListProps)}
                            item={item}
                            key={key}
                        />
                    );
