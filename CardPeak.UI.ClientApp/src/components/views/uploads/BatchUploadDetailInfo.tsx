...
const BatchUploadDetailInfoLayout = (
    props: BatchUploadDetailInfoProps
) => {
    return (
        <Row>
            ...
            <Col md={3} lg={2} sm={2} xsHidden={props.isHeader}>
                {props.isHeader
                    ? 'bank'
                    : props.batchUpload.bank.description}
            </Col>
            <Col md={4} lg={4} sm={5} xsHidden={props.isHeader}>
                {props.isHeader
                    ? 'file'
                    : props.batchUpload.originalFileName}
            </Col>
            <Col mdHidden lg={1} smHidden xsHidden={props.isHeader}>
                {props.isHeader
                    ? 'start'
                    : props.batchUpload.processStartDateTime
                    ? dateTimeFormat(
                          props.batchUpload.processStartDateTime
                      )
                    : null}
            ...
            <Col mdHidden lg={1} smHidden xsHidden={props.isHeader}>
                {props.isHeader
                    ? 'end'
                    : props.batchUpload.processEndDateTime
                    ? dateTimeFormat(
                          props.batchUpload.processEndDateTime
                      )
                    : null}
            ...
            <Col md={1} lg={1} sm={1} xsHidden={props.isHeader}>
                {props.isHeader
                    ? 'rows'
                    : props.batchUpload.processedRecords
                    ? props.batchUpload.processedRecords
                    : null}
            </Col>
            <Col md={1} lg={1} sm={1} xsHidden={props.isHeader}>
                ...
                {!props.isHeader &&
                props.batchUpload.hasErrors === true ? (
                    <span className="text-highlight text-danger">
                        No
                    </span>
                ) : null}
                {!props.isHeader &&
                props.batchUpload.hasErrors === false ? (
...
const BatchUploadDetailInfo = (props: BatchUploadDetailInfoProps) => {
    return (
        <GridList
            header={<BatchUploadDetailInfoLayout isHeader={true} />}>
            <Panel className="panel-row">
                <BatchUploadDetailInfoLayout
                    batchUpload={props.batchUpload}
                    isHeader={false}
                />
            </Panel>
        </GridList>
