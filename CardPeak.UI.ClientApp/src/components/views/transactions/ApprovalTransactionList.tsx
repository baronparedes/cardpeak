...
type ApprovalTransactionDataList = new () => DataListFiltered<
    CardPeak.Entities.ApprovalTransaction
>;
...
class DeleteTransactionButton extends React.Component<
    DataItemProps<CardPeak.Entities.ApprovalTransaction> &
        ApprovalTransactionListProps,
    DeleteTransactionButtonState
> {
    constructor(
        props: DataItemProps<CardPeak.Entities.ApprovalTransaction> &
            ApprovalTransactionListProps
    ) {
        super(props);
        this.state = {
    ...
    render() {
        return (
            <ConfirmButton
                ...
                <p>
                    You are about to
                    <strong className="text-highlight text-danger">
                        {' '}
                        delete{' '}
                    </strong>
                    transaction:{' '}
                    <span className="text-highlight">
                        {this.props.item.client}
                    </span>
                </p>
                <p>
                    <label className="text-muted text-small spacer-right">
                        credited to
                    </label>
                    <span className="text-highlight text-small">
                        {concat(
                            this.props.item.agent.firstName,
                            ' ',
                            this.props.item.agent.lastName
                        )}
                    </span>
                </p>
...
const ApprovalTransactionListRowLayout = (
    props: DataItemProps<CardPeak.Entities.ApprovalTransaction> &
        ApprovalTransactionListProps
) => {
    return (
        <Row>
            ...
            <Col md={1} lg={1} sm={1} xsHidden={props.isHeader}>
                {props.isHeader
                    ? 'bank'
                    : props.item.bank.shortDescription}
            </Col>
            <Col md={1} lg={1} sm={1} xsHidden={props.isHeader}>
                {props.isHeader
                    ? 'card category'
                    : props.item.cardCategory.shortDescription}
            </Col>
            ...
            <Col md={2} lg={2} sm={2} xsHidden={props.isHeader}>
                {props.isHeader
                    ? 'upload date'
                    : dateFormat(props.item.createdDate)}
            </Col>
            <Col md={2} lg={2} sm={2} xsHidden={props.isHeader}>
                {props.isHeader
                    ? 'approval date'
                    : dateFormat(props.item.approvalDate)}
            </Col>
            {props.hideAmount ? (
                <Col md={2} lg={2} sm={2} xsHidden={props.isHeader}>
                    {props.isHeader ? (
...
class ApprovalTransactionList extends React.Component<
    DataListProps<CardPeak.Entities.ApprovalTransaction> &
        ApprovalTransactionListProps,
    ApprovalTransactionListState
> {
    constructor(
        props: DataListProps<CardPeak.Entities.ApprovalTransaction> &
            ApprovalTransactionListProps
    ) {
        super(props);
        this.state = {
            ...
            <div>
                <ApprovalTransactionDataList
                    predicate={(_, searchString) => {
                        ...
                        const productMatched = _.cardCategory.description
                            .toLowerCase()
                    ...
                    renderHeader={() => {
                        return (
                            <ApprovalTransactionListRowLayout
                                isHeader
