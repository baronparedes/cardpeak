...
type TeamDataListFiltered = new () => DataListFiltered<
    CardPeak.Entities.Team
>;
...
class TeamListRowLayout extends React.Component<
    DataItemProps<CardPeak.Entities.Team> & TeamListProps,
    { isDeleting: boolean }
> {
    constructor(
        props: DataItemProps<CardPeak.Entities.Team> & TeamListProps
    ) {
        super(props);
        this.state = {
            isDeleting: false
...
const TeamList = (
    props: DataListProps<CardPeak.Entities.Team> & TeamListProps
) => {
    return (
        <div>
            <TeamDataListFiltered
                predicate={(team, searchString) => {
                    const nameMatch =
                        team.name.toLowerCase().indexOf(searchString) >=
                        0;
                    return nameMatch;
                }}
                ...
                renderHeader={() => {
                    return <TeamListRowLayout isHeader />;
                }}
                renderItem={(item, key) => {
                    return (
                        <TeamListRowLayout
