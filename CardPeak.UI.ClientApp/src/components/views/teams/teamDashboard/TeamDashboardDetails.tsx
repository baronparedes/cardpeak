...
type AgentPerformanceMetricsDataList = new () => DataListFiltered<
    CardPeak.Entities.TeamDashboardDetail
>;
...
class AgentPerformanceMetricsRowLayout extends React.Component<
    DataItemProps<CardPeak.Entities.TeamDashboardDetail> &
        TeamDashboardDetailsProps,
    AgentPerformanceMetricsRowLayoutState
> {
    constructor(
        props: DataItemProps<CardPeak.Entities.TeamDashboardDetail> &
            TeamDashboardDetailsProps
    ) {
        super(props);
        this.state = {
    ...
    handleOnConfirm = () => {
        if (this.props.onRemoveAgent) {
            this.setState({ removingAgent: true }, () => {
                this.props.onRemoveAgent(
                    this.props.item.teamPlacement,
                    () => {
                    ...
                    {this.props.isHeader ? (
                        ...
                        <div className="spacer-bottom">
                            <span className="text-highlight text-large">
                                {this.props.item.teamPlacement.agent
                                    .firstName +
                                    ' ' +
                                    this.props.item.teamPlacement.agent
                                        .lastName}
                            ...
                            <span className="float-right">
                                <ConfirmButton
                                    ...
                                    <p>
                                        ...
                                        <br />
                                        <span className="text-muted spacer-right">
                                            agent:
                                        </span>
                ...
                <Col md={2} xs={2}>
                    {this.props.isHeader ? null : (
                        <div className={'text-right align-center'}>
                            <small className="text-muted spacer-right">
                                total
                            </small>
                            <ApprovalMetric
                                metric={this.props.item.totalApprovals}
                ...
                <AgentPerformanceMetricsDataList
                    predicate={(metric, searchString) => {
                        const firstNameMatch =
                            metric.teamPlacement.agent.firstName
                                .toLowerCase()
                                .indexOf(searchString) >= 0;
                        const lastNameMatch =
                            metric.teamPlacement.agent.lastName
                                .toLowerCase()
                                .indexOf(searchString) >= 0;
                    ...
                    renderHeader={() => {
                        return (
                            <AgentPerformanceMetricsRowLayout
                                isHeader
                            />
                        );
                    }}
                    renderItem={(item, key) => {
                        return (
                            <AgentPerformanceMetricsRowLayout
                                item={item}
                                key={key}
                                onRemoveAgent={props.onRemoveAgent}
                            />
                        );
                    }}
                    data={props.details}
