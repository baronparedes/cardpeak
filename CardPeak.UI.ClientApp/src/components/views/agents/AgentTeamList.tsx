...
import { GridList, ListNoRecordsRow } from '../../layout';
...
class AgentTeamDetailRowLayout extends React.Component<
    AgentTeamDetailRowLayoutProps,
    undefined
> {
    handleOnRemoveAccount = () => {
        if (this.props.onRemoveTeamPlacement) {
            this.props.onRemoveTeamPlacement(
                this.props.teamPlacement.teamId
            );
        }
    };
    render() {
        return (
            <Row>
                <Col md={10} lg={10} sm={11} xs={10}>
                    {this.props.isHeader
                        ? 'teams'
                        : this.props.teamPlacement.team.name}
                </Col>
                <Col>
...
class AgentTeamList extends React.Component<
    AgentTeamListProps,
    undefined
> {
    ...
    render() {
        return (
            <div>
                ...
                <GridList
                    header={<AgentTeamDetailRowLayout isHeader />}>
                    {this.props.agent.teamPlacements &&
                    this.props.agent.teamPlacements.length > 0 ? (
