...
type AgentMetricsDataListFiltered = new () => DataListFiltered<
    CardPeak.Entities.AgentApprovalMetric
>;
...
const AgentMetricsRowLayout: React.StatelessComponent<DataItemProps<
    CardPeak.Entities.AgentApprovalMetric
>> = props => {
    return (
        <Row>
            ...
            <Col
                ...
                {props.isHeader ? (
                    ...
                    <Currency currency={props.item.accountBalance} />
                )}
            </Col>
            <Col
                sm={3}
                ...
                {props.isHeader ? (
                    ...
                    <Currency
                        noCurrencyColor
                        currency={props.item.savingsBalance}
                    />
                )}
            </Col>
        </Row>
...
const AgentMetricsTotals: React.StatelessComponent<CardPeak.Models.MetricsModel> = props => {
    return (
        <Row>
            <Col sm={6}>
                <TotalApprovedMetrics
                    totalApprovals={props.agentMetrics.totalApproved}
                />
            </Col>
            <Col sm={6}>
                <Panel className="panel-label-dashboard">
                    <DashboardLabel
                        ...
                        label="balance"
                        metrics={props.agentMetrics.totalBalance}
                        isCurrency
                    />
                    <DashboardLabel
                        className="pull-right"
                        label="savings"
                        metrics={props.agentMetrics.totalSavings}
                        isCurrency
                        noCurrencyColor
                    />
                </Panel>
...
class AgentMetricsContainer extends React.Component<
    CardPeak.Models.MetricsModel & AgentMetricsContainerDispatchProps,
    AgentMetricsContainerState
> {
    constructor(
        props: CardPeak.Models.MetricsModel &
            AgentMetricsContainerDispatchProps
    ) {
        super(props);
        this.state = {
    ...
    handleOnChange = (e: any) => {
        this.setState(
            ...
            () => {
                this.filterMetrics(
                    this.props.agentMetrics.agentApprovalMetrics
                );
            }
        );
    };
    filterMetrics(data: CardPeak.Entities.AgentApprovalMetric[]) {
        let filteredData = data.filter(item => {
            ...
            const result =
                ...
                showNegativeBalance ||
                showPayouts ||
                (this.state.showNegativeBalance &&
                    this.state.showPayouts &&
                    this.state.showIncentives &&
                    this.state.showZeroBalance);
    ...
    componentDidMount() {
        ...
        this.props.actions.getAgentMetricsStart(
    ...
    componentWillReceiveProps(nextProps: CardPeak.Models.MetricsModel) {
        if (
            this.props.agentMetrics.agentApprovalMetrics !=
            nextProps.agentMetrics.agentApprovalMetrics
        ) {
            this.filterMetrics(
                nextProps.agentMetrics.agentApprovalMetrics
            );
            ...
            <div>
                ...
                <div>
                    ...
                    <Grid>
                        <Row>
                            <AgentMetricsTotals
                                agentMetrics={this.props.agentMetrics}
                            />
                        </Row>
                        <Row className="text-right">
                            <FormGroup>
                                <Checkbox
                                    ...
                                    checked={this.state.showZeroBalance}
                                    onChange={this.handleOnChange}
                                    inline>
                                    <span className="text-highlight money-zero-balance">
                                        zero balance
                                    </span>
                                </Checkbox>
                                <Checkbox
                                    name="showNegativeBalance"
                                    checked={
                                        this.state.showNegativeBalance
                                    }
                                    onChange={this.handleOnChange}
                                    inline>
                                    <span className="text-highlight money-debit">
                                        negative balance
                                    </span>
                                </Checkbox>
                                <Checkbox
                                    name="showPayouts"
                                    checked={this.state.showPayouts}
                                    onChange={this.handleOnChange}
                                    inline>
                                    <span className="text-highlight money-credit">
                                        payout
                                    </span>
                                </Checkbox>
                            </FormGroup>
