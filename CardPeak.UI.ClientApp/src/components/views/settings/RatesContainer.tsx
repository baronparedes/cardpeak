...
class RatesContainer extends React.Component<
    CardPeak.Models.RatesModel & Props & DispatchProps,
    State
> {
    constructor(
        props: CardPeak.Models.RatesModel & Props & DispatchProps
    ) {
        super(props);
        this.state = {
            bank: undefined,
    ...
    handleErrors = () => {
        ...
        if (!!!this.state.amount || this.state.amount === 0)
            errors.amount = '*';
        if (!!!this.state.bank || this.state.bank.referenceId === 0)
            errors.bankId = '*';
        if (
            !!!this.state.cardCategory ||
            this.state.cardCategory.referenceId === 0
        )
            errors.cardCategoryId = '*';
    ...
    handleOnClickAddRate = (e: any) => {
        ...
        if (
            ...
            !!this.state.errors.cardCategoryId
        ) {
            return;
        }
        let rate = {
            ...
            cardCategoryId: parseInt(
                this.state.cardCategory.referenceId.toString()
            ),
    ...
    handleOnSelectReference = (
        ref: CardPeak.Entities.Reference,
        name: any
    ) => {
        this.setState({ postingRatesError: undefined });
        let errors = this.state.errors;
    ...
    handleOnClickSaveRates = () => {
        if (this.props.defaultRate) {
            this.props.actions.postDefaultRatesStart(
                this.props.typeId,
                this.props.rates,
                null,
                e => {
            ...
            this.props.actions.postRatesStart(
                this.props.agentId,
                this.props.rates,
                null,
                e => {
    ...
    handleOnConfirmDefaultRate = () => {
        this.props.actions.applyDefaultRateStart(
            this.state.selectedDefaultRate,
            this.props.selectedAgentId
        );
    };
    componentDidMount() {
        if (this.props.defaultRate) {
            this.props.actions.selectDefaultRateStart(
                this.props.selectedTypeId
            );
        } else {
            this.props.actions.selectAgentStart(
                this.props.selectedAgentId
            );
        }
    }
    componentWillReceiveProps(nextProps: Props) {
        if (nextProps.defaultRate) {
            if (
                this.props.selectedTypeId !== nextProps.selectedTypeId
            ) {
                this.props.actions.selectDefaultRateStart(
                    nextProps.selectedTypeId
                );
                this.handleOnClearRate();
            }
        } else {
            if (
                this.props.selectedAgentId !== nextProps.selectedAgentId
            ) {
                this.props.actions.selectAgentStart(
                    nextProps.selectedAgentId
                );
                this.handleOnClearRate();
            }
        }
            ...
            <Grid fluid>
                                        ...
                                        <ButtonGroup
                                            ...
                                            this.props
                                                ...
                                                <ConfirmButton
                                                    ...
                                                    onPreventConfirm={
                                                        this
                                                            .handleOnPreventToggleDefaultRate
                                                    }
                                                    onConfirm={
                                                        this
                                                            .handleOnConfirmDefaultRate
                ...
                <Row>
                    ...
                    <Col sm={12} xs={12} md={12} lg={12}>
                        <ErrorLabel
                            error={this.state.postingRatesError}
                        />
                    </Col>
                </Row>
            </Grid>
