...
const RateListRowActions = (
    props: { rate: CardPeak.Entities.Rate } & RateListProps
) => {
    let deleteButton = (
        <Button
            onClick={() => {
                props.onDeleteRate(props.rate);
            }}
            bsStyle="danger"
            bsSize="sm">
...
const RateListRowLayout = (
    props: DataItemProps<CardPeak.Entities.Rate> & RateListProps
) => {
    return (
        <Row className="agent-item">
            ...
            <Col sm={3} xsHidden={props.isHeader}>
                {props.isHeader
                    ? 'category'
                    : props.item.cardCategory.description}
            </Col>
            <Col sm={2} xsHidden={props.isHeader}>
                {props.isHeader ? (
                    ...
                    <HighlightedSpan
                        className="currency"
                        value={currencyFormat(props.item.amount)}
                    />
                )}
            </Col>
            <Col sm={2} xsHidden={props.isHeader}>
                {props.isHeader ? (
                    ...
                    <HighlightedSpan
                        className="currency"
                        value={currencyFormat(props.item.savingsAmount)}
                    />
                )}
            </Col>
            <Col sm={2}>
...
const RateList = (
    props: DataListProps<CardPeak.Entities.Rate> & RateListProps
) => {
    return (
        <div>
            <RateDataList
                predicate={(rate, searchString) => {
                    const bankMatched = rate.bank.description
                        .toLowerCase()
                        .startsWith(searchString);
                    const categoryMatched = rate.cardCategory.description
                        .toLowerCase()
                        .startsWith(searchString);
                    let shortDescriptionMatched = false;
                    if (rate.bank.shortDescription) {
                        shortDescriptionMatched =
                            rate.bank.shortDescription.toLowerCase() ===
                            searchString;
                    }
                ...
                onGetKey={rate =>
                    rate.bank.description +
                    rate.cardCategory.description
                }
                renderHeader={() => {
                    return <RateListRowLayout isHeader />;
                }}
                renderItem={(item, key) => {
                    return (
                        <RateListRowLayout
