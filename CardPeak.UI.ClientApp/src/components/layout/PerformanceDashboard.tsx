...
const Performance = (props: PerformanceDashboardProps) => {
    let caret = null;
    if (
        props.compareUnits > props.perf.value &&
        props.perf.value !== 0
    ) {
        caret = (
            <i className="perf-down fa fa-caret-down spacer-left"></i>
        );
    ...
    return (
        <div className="spacer-bottom-2x">
            <label
                className={concat(
                    'text-label text-muted dashboard-label'
                )}>
                {props.perf.key}
            </label>
            {!props.hideValue && (
            ...
            <span className="block">
                {props.hideAmount ? null : props.perf.amount === 0 ? (
                    <span>-</span>
                ) : (
                    <span className="currency text-highlight text-medium">
                        {currencyFormat(roundOff(props.perf.amount))}
                    </span>
                )}
            </span>
        </div>
                ...
                {props.performance.map(_ => {
                    ...
                    let perf = (
                        <Performance
                            ...
                            compareUnits={
                                props.performance[compareRow].value
                            }
                            {...props}
                        />
                    );
