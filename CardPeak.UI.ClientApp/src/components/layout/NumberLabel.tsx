...
export const Currency = (props: CurrencyProps) => {
    ...
    if (props.currency) {
        currencyColor = props.noCurrencyColor
            ? ''
            : props.currency < 0
            ? 'amount-debit'
            : props.currency > 0
...
export const ApprovalMetric = (props: {
    metric: number;
    className?: string;
}) => {
    return (
        <label
            className={classNames('text-highlight', props.className)}>
            {roundOff(props.metric)}
        </label>
    );
};
