...
namespace CardPeak.WebAPI.Controllers
{
	...
	public sealed class MetricsController : ApiController
	{
		...
		[HttpGet]
		[Route("agents/disbursement")]
		public IHttpActionResult GetAgentDisbursement([FromUri]DateTime startDate)
		{
			var result = this.MetricsService.GetAgentDisbursementMetrics(startDate);
			if (result == null)
			{
				return this.NotFound();
			}
